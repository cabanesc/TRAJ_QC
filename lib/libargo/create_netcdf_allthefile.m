...
if isfield(VARS,'dimorder')==0 % si rien n'est precise
    if isempty(REC)==0
        VARS = check_FirstDimArray_is(VARS,REC);
    end
    VARS.dimorder = 'C';
end
...
if strcmp(VARS.dimorder,'F')
    isneedflip=0;
    if isempty(REC)==0
        VARS = check_LastDimArray_is(VARS,REC); % verifie et permute si necessaire
    end
else
    isneedflip=1;
    if isempty(REC)==0
        VARS = check_FirstDimArray_is(VARS,REC); % verifie et permute si necessaire
    end
end

% verifie que les NaN sont remplaces par la valeur fillvalue
VARS=replace_nan_byfill(VARS);
