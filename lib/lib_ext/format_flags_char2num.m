            ...
            if thisiscar==1      % test si c'est une chaine de caractère
                ...
                if length(size(Pr.(oneChamp).data))>2       % test si c'est un tableau de 2 dim
                   % warning('Does not accept array of dim >2')
                    ...
                    isalphanum = sumel(isstrprop(poubflag,'digit')) == numel(poubflag); %=1 si tableau alphanumerique
                    clear poubflag
                    ...
                    if isalphanum==1  % test si tableau alphanumerique
                        ...
                        if size1<=size2  % cc 15/02/2021 accelère la fonction si size1 tres grand
                            ...
                            for i=1:size2                              % boucle sur chaque profil
                                ...
                                if sum(isnofill)~=0
                                    ...
                                    Pr.(oneChamp).data(isnofill,i)=single(flagnum)';
...
%keyboard
if isfield(Pr,'fillisnan')
   if(Pr.fillisnan==1)
      Pr.fillisnan=0;
      Pr=replace_fill_bynan(Pr);
end
end