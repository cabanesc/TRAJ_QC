...
parameters.TIME_DIFF_CYCLE = .1; % in day % Max value for difference between metadata and estimated median for cycle duration
parameters.PRESS_PARK_DIFF_BATHY_QC = 100; % Put QC=4 if pressure is xxx dbar below the bathymertry
parameters.PRESS_PARK_DUMB = 2800; % Threshold for dumb park pressure, put QC=4
...
parameters.TIME_DIFF_FLLOCS_IR = 10; % in hour % Thershold for acceptable differnce between first and last location for Argos transmission system (within one cycle)
