% ------------------------------------------------------------------------------
% Passage du crit�re JAMSTEC sur un ensemble de positions Argos de surface
% (avec prise en compte du flag � 4).
%
% SYNTAX :
...
%
% INPUT PARAMETERS :
%   a_date      : date des positions Argos observ�es
%   a_longitude : longitude des positions Argos observ�es
%   a_latitude  : latitude des positions Argos observ�es
%   a_posAcc    : classe des positions Argos observ�es
%   a_posQc     : flag des positions Argos observ�es
%   a_juldLast  : date de la derni�re bonne positions Argos du cycle
%                 pr�c�dent
%   a_longLast  : longitude de la derni�re bonne positions Argos du cycle
%                 pr�c�dent
%   a_latLast   : latitude de la derni�re bonne positions Argos du cycle
%                 pr�c�dent
%   a_fidOut    : Id du fichier log
%
% OUTPUT PARAMETERS :
%   o_date      : date des positions Argos (�ventuellement reclass�es)
%   o_longitude : longitude des positions Argos (�ventuellement reclass�es)
%   o_latitude  : latitude des positions Argos (�ventuellement reclass�es)
%   o_posAcc    : classe des positions Argos (�ventuellement reclass�es)
%   o_posQcIn   : flag original des positions Argos (�ventuellement reclass�es)
%   o_posQcOut  : flag final des positions Argos (�ventuellement reclass�es)
%   o_idBadPos  : indices des positions Argos rejet�es (flag 3 ou 4)
%
% EXAMPLES :
%
...
function [o_date, o_longitude, o_latitude, o_posAcc, o_posQcIn, o_posQcOut ...
   o_speed,o_idBadPos, Little_Time, Little_Time_Sp, deltaTime_Sp, normal_Time_Sp] = check_argos_positions_koba_all( ...
   a_date, a_longitude, a_latitude, a_posAcc, a_posQc, ...
   a_juldLast, a_longLast, a_latLast, a_fidOut)
...
normal_Time_Sp = [];
deltaTime_Sp = [];
% vitesse maximale autoris�e (m/s)
MAX_VEL = 5;
idd=0;
idd2=0;
...
% on convertit les classes Argos en num�rique
...
% precision affect�e aux positions "0", "A", "B" ou "Z"
precision(5) = 1500;
precision(6) = 1500;
precision(7) = 1500;
precision(8) = 1500;

% classement temporel des donn�es
%keyboard
...
%keyboard
      ...
      if(distance(order(maxId))<dderr & deltaTime(order(maxId)) <= 0.00069444440305233 & flag(order(maxId))==1)   %%60s
          idd=idd+1;
          Little_Time(idd) = order(maxId);
          Little_Time_Sp(idd) = speed(order(maxId));
          deltaTime_Sp(idd) = deltaTime(order(maxId));
     % keyboard
      end
      
      if(distance(order(maxId))<dderr & deltaTime(order(maxId)) > 0.000347222201526165 &  flag(order(maxId))==1)
          idd2=idd2+1;
          normal_Time(idd2) = order(maxId);
          normal_Time_Sp(idd2) = speed(order(maxId));
     % keyboard
      end
...
% return;
% 
% % conversion jour julien -> jour gr�gorien par JAMSTEC (pour effectuer les
% % comparaisons)
