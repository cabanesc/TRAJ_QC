...
for ilist=1:length(Liste_Float)   % add boucle cc 02/11/202
    ...
    file3d = [CONF.DIR_DATA 'alerts/'  CONF.liste_rep_alerte{ilist} '/Parking_pressure_mission.txt'];
    ...
    fid3d = fopen(file3d,'w+');
    fprintf(fid3d,'%s\n',['Float number,Parking pressure Meta, Retained parking pressure, Cycles']);
                ...
                if length(unique(T.cycle_number_index.data))==length((T.cycle_number_index.data))   % cc 03/05/2022 ajout deuxieme test pour eliminer flotteurs dont les fichiers traj sont completement pourris
                    ...
                    if isempty(idLoc)==0
                        ...
                        if ~isempty(idLoc) % correction cc 03/11/2020 on ne passe pas si pas de loc
                            ...
                            fprintf('%s\n',[' ']);
                            fprintf('%s\n',[ 'Float number,Parking pressure Meta, Retained parking pressure, Cycles:'])
                            fid3d=fopen(file3d,'a');
                            clear  allpark_str
                            for m=1:length(park_prof)
                                if m<=length(M.ParkPressure)
                                    allpark_str{m}=[num2str(M.ParkPressure(m)) ', ' num2str(-park_prof(m))];
                                else
                                    allpark_str{m}=['n/a, ' num2str(-park_prof(m))];
                                end
                            end
                            unique_park_prof=unique(allpark_str,'stable');
                            for iparkp=1:length(unique_park_prof)
                                iuni=find(findstr_tab(allpark_str,unique_park_prof{iparkp}));
                                TcycunM_i=[];
                                ...
                                if m<=length(M.ParkPressure)
                                    park_meta_str=num2str(M.ParkPressure(m));
                                else
                                    park_meta_str='n/a';
                                end
                                fprintf('%s\n',[ floatname ', '  unique_park_prof{iparkp} ', ' strindex]);
                                fprintf(fid3d,'%s\n',[ floatname ', '  unique_park_prof{iparkp} ', ' strindex]);
                            ...
                            if (sum(idCyc) > 0)
                                ...
                                if  id>1 &  (dlocPosition_qc_sorted<4)&(dlocPosition_qc_sortedend<4) & ~isnan(dureeCycle) & sum(idCycprec{id})>0
                                    ...
                                    if distanceprof> dureeCycle*(cycles_sorted(id)-cycles_sorted(id-1))*(24*60*60)*PARAM.SPEED_MAX % borne a adapter en fonction de la duree du cycle (vitesse maximale du flotteur : 3 m/s)
                                        %cc 02/02/2021
                                        if distanceprof> dureeCycle*(cycles_sorted(id)-cycles_sorted(id-1))*(24*60*60)*3                                          
                                            verylargedisp=1;
                                        end
                                        fid_alerte=fopen(file_alerte,'a');
                                ...
                                if length(locLon)==1& (verylargedisp==1) % pas de test ranges ou koba possible(LocLon=1), test ci dessous mieux que rien                                   
                                   idKo=1;
                                   o_idBadPos=1;
                                   
                                   idCyclesBA = find((T.cycle_number.data>=cycles_sorted(id)-2&T.cycle_number.data<=cycles_sorted(id)+2)&~isnan(T.longitude.data)&~isnan(T.latitude.data)&T.cycle_number.data~=cycles_sorted(id));
                                   meanlat = mean(T.latitude.data(idCyclesBA));
                                   meanlon = mean(T.longitude.data(idCyclesBA));
                                   stdlat = std(T.latitude.data(idCyclesBA));
                                   stdlon = std(T.longitude.data(idCyclesBA));
                                   isin = abs(locLon_sorted-meanlon)<=3*stdlon|abs(locLat_sorted-meanlat)<=3*stdlat; 
                                   idKo(isin)=[];
                                   o_idBadPos(isin)=[];
                                end
                                   
                                
                                if  (~isempty(idKo))&~isempty(o_idBadPos) % if ranges test fail, use koba to flag bad positions => erreur locpos d'altran
                                     
                                    % dans le cas ou simplement deux posistions (iridium) par cycle essaie de determiner quelle 
                                    % sont la/les mauvaises (koba ne le fait pas bien)
                                    if length(locLon_sorted)==2
                                        
                                        idCyclesBA = find((T.cycle_number.data==cycles_sorted(id)-1|T.cycle_number.data==cycles_sorted(id)+1)&~isnan(T.longitude.data)&~isnan(T.latitude.data));
                                        meanlat = mean(T.latitude.data(idCyclesBA));
                                        meanlon = mean(T.longitude.data(idCyclesBA));
                                        stdlat = std(T.latitude.data(idCyclesBA));
                                        stdlon = std(T.longitude.data(idCyclesBA));
                                        isin = abs(locLon_sorted-meanlon)<=3*stdlon&abs(locLat_sorted-meanlat)<3*stdlat;
                                       % meme si une position est bonne, koba en detecte generalement 2 mauvaises
                                       % remarque si stat pas bonne (std tres faible isin=[0 0]
                                       % ou si donees des cycles avant/apres sont mauvaises ou trop loin (isin=[1 1]), on reste avec les resultats de koba. 
                                        if sum(isin)==1&length(o_idBadPos)==2 
                                            o_idBadPos(isin)=[];
                                        end
                                        % si les deux positions sont
                                        % mauvaises, koba peut n'en détecter qu'une (si elles ne sont pas trop eloignees l'une de l'autre)
                                        if sum(isin)==0&length(o_idBadPos)==1
                                            o_idBadPos=[1;2];
                                        end
                                    end
